...
class RBFRegression():
    ...
    1) For the regularization, and predicting, calculate X as a whole, don't split it up even though X represents all the points
    2) The bigger picture is that, we want to find points that are very different to its surroundings and replace it with the average of its
       surroundings -> then we get the actual value
    3) fit and regularize data -> then predict values
